...
object Brack{
  ...
  val combs = Array(Array((0,2),(1,2),(2,0)),
                    Array((0,0),(0,1),(1,1)),
                    Array((1,0),(2,1),(2,2)))
  ...
  /* Functions below here need to be implemented */
	...
	def PossibleRec(w: Array[Int], i: Int, j: Int, z: Int): Boolean = {
	  ...
	  if (j == i+1) {
            ...
            while (k < j && !found) {
              var t = 0
              while (t < 3 && !found) {
                found = PossibleRec(w,i,k,combs(z)(t)._1) && PossibleRec(w,k,j,combs(z)(t)._2)
                t += 1
              }
              k += 1
            }
	...
	def NumberRec(w: Array[Int], i: Int, j: Int, z:Int): Int = {
	  ...
	  if (j == i+1) {
            ...
            while (k < j) {
              var t = 0
              while (t < 3) {
                cnt += NumberRec(w,i,k,combs(z)(t)._1) * NumberRec(w,k,j,combs(z)(t)._2)
                t += 1
              }
              k += 1
            }
	...
	// TASK 3
        ...
        Accounitng for the overhead the result roughly 7-uples.
        ...
        of the reccursion tree and it will be exponential.
        If we, say, visit half on each step, we get:
        T(n) = 3.5^n * 4
        So, for n = 15 we wpuld expect about 0.6 seconds.
          ...
          if (j == i+1) {
            ...
            while (k < j) {
              ...
              while (t < 3) {
                ...
                if (poss(i)(k)(lsym) && poss(k)(j)(rsym)) exp(i)(j)(z) = Node(exp(i)(k)(lsym),exp(k)(j)(rsym))
                t += 1
              }
              k += 1
            }
