...
public class BeastAttackOptimized
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        System.out.println(df2.format(uses * 100.0 / trials) + "% IV use rate.");
    }
    ...
    static void decryptMessage(byte[] message, int messageLength, boolean fancy) throws Exception
    {
        ...
        {
            for (int i = 0; i < 256; i++) byteUsed[i] = false;
            ...
            do
            {
                ...
                prefix[blockSize - 1] = (byte) (xordPrefix[blockSize - 1] ^ xorWith[blockSize - 1]);
                int byteIndex = prefix[blockSize - 1] & 0xFF;
                if (!byteUsed[byteIndex]) uses++;
                byteUsed[byteIndex] = true;
                while (byteUsed[(int) allBytes[i]]) i++;
            }
            while (!checkMatchBlock(ciphertext, oldBlock, 1));
            message[byteNum] = prefix[blockSize - 1];
            if (fancy) System.out.print("\b" + (char) message[byteNum]);
        }
    }
    ...
    static void callEncryptWithIV(byte[] prefix , byte[] ciphertext) throws Exception
    {
        ...
        while (!checkMatchAlmostBlock(ciphertext, predictedIV, 0));
        if (ciphertext[blockSize - 1] == predictedIV[blockSize - 1]) matches++;
        prefix[blockSize - 1] = (byte) (xordPrefix[blockSize - 1] ^ ciphertext[blockSize - 1]);
    }
    ...
    // Returns whether the first blockSize-1 bytes match, from starts from startBlock
    static boolean checkMatchAlmostBlock(byte[] from, byte[] to, int startBlock)
    {
        for (int i = 0; i < blockSize - 1; i++)
        {
            if (from[startBlock * blockSize + i] != to[i]) return false;
