...
type code =
  ...
  | POP                         (* Pops the top value on the stack *)
...
let enable_stack_sanity = false (* Fix later, provided code is buggy *)
...
let check_and_output code =
  ...
  let note_label lab def d =
    try 
      ...
      if enable_stack_sanity && d >= 0 && d <> d1 then
  ...
  let rec scan d = 
    match !tail with
        [] -> 
          if enable_stack_sanity && d <> 0 then insane "stack not empty at end" []
      | x :: _ ->
          let need a =
            if enable_stack_sanity && d < a then 
